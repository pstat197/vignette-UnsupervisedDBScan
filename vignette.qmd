...
The code below loads the necessary libraries and the data from the data folder
...
Furthermore, it is important to note that this was a synthetically generated dataset. Real world dataset might have problems dealing with correlation, missing data, class imbalance (although all four clustering methods perform relatively well with class imbalance datasets as well). If you have highly correlated dataset you might expect a wider spread in the clusters. Our dataset had a lot of correlation too (see the heatmap in the [EDA section](https://github.com/pstat197/vignette-Supervised-Unsupervised-Clustering/blob/main/Scripts/drafts/ExploratoryAnalysis.Rmd)), however, the methods we deployed could correctly identify the clusters. With a lot of correlation you might expect wider clusters since the model will be confusing the correlation between variables more than they are actually used for predicting the personality type. This exact reason is why we see wider clusters when we have correlation. 

If the boundaries are getting blurry when you have correlation, please refer to other resources to deal with your correlation accordingly. Some suggestions from us are deploying a variable selection method before continuing with clustering. Some common variable selection methods widely used are Lasso (if the data is linear), Randomized Lasso, Elastic Net, Stepwise selection, Graphical Modelling, Principle Component Analysis (PCA was done in the code for visualizing some of the clustering methods we used as well), and even some Tree-Based importance methods. 
